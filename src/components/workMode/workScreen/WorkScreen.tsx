...
import {
    SCRAP_BASELINE_BOTTOM_WU,
    SCRAP_SIZE_WU,
    SCRAP_BIN_SIZE_WU,
    SCRAP_BIN_POSITION_WU,
...
const WorkScreen: React.FC<WorkScreenProps> = ({ updateCredits, installedApps }) => {
        ...
        if (gameMode === "workMode") {
            ...
            setBarriers([
                ...
                },
                {
                    ...
                    position: {
                        x: 0.75, // Center X position (world units, 0-20)
    ...
    const checkScrapSpawning = useCallback(
        (currentTime: number) => {
    ...
    const updateScrapPositions = useCallback(
        (deltaTime: number) => {
    ...
    return (
        <div className="work-screen">
            ...
            <AssemblyLine />
            <ScrapBin ref={dropZoneRef} style={binScreenPosition} />
            ...
            {scrapItems.map((item) => (
                <ScrapItem key={item.id} scrap={item} style={item.style} draggableProps={getDraggableProps(item.id)} />
            ))}
        </div>
    );
};
